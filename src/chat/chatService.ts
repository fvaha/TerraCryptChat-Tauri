...
export class ChatService {
    ...
    private async getCurrentUserToken(): Promise<string | null> {
        try {
            ...
            console.error('[ChatService] Failed to get current user token:', error);
            return null;
        }
    }

    async performDeltaChatSync(): Promise<void> {
        try {
            console.log('[ChatService] Performing delta chat sync...');
            const token = await this.getCurrentUserToken();
            if (token) {
                await this.chatsDeltaUpdate(token);
            }
            console.log('[ChatService] Delta chat sync completed');
        } catch (error) {
            console.error('[ChatService] Delta chat sync failed:', error);
            throw error;
        }
    }
}
